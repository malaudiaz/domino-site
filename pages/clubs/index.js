...
export default function ClubsPage() {
  ...
  const fetchData = async () => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}club/${profile.id}?page=${page}&per_page=${rowsPerPage}`;
  ...
  const delItem = async (id) => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}club/${id}`;
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "¿ Eliminar Clubs ?",
      text: "¿ Deseas eliminar éste Clubs ?",
      icon: "question",
        ...
        >
          ...
          <Link href="/clubs/create" >
            <a className="btn btn-primary btn-sm">
              <i className="bi bi-plus-lg"></i> Crear Club
            </a>
          </Link>

        </div>

        <ClubsTable clubs={clubs} onDelete={handleDelete} mode={mode}/>
